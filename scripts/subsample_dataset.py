...
parser.add_argument("--dataset_dir", type=str, required=True, help="Path to the dataset directory containing the scenes: 00000, 000001, etc.")
parser.add_argument("--num_scene_images", type=int, help="Number of images to subsample from each scene.")
...
def subsample_images(num_images, scene_gt, scene_gt_info):
    ...
    for im_id in im_ids:
        ...
        descs_inds = [i for i, src_im_id in enumerate(gt_src_ims_list)
                      if src_im_id == im_id]

        gt_descs_im = gt_descs[np.array(descs_inds), :]
        im_desc = np.concatenate((
            np.mean(gt_descs_im, axis=0),
            np.std(gt_descs_im, axis=0)
        ))
...
scene_ids = [scene for scene in os.listdir(p["dataset_dir"]) if os.path.isdir(os.path.join(p["dataset_dir"], scene)) and scene.isdigit()]
...
for scene_id in scene_ids:
    ...
    scene_gt = inout.load_json(os.path.join(args.dataset_dir, scene_id, "scene_gt.json"))
    scene_gt_info = inout.load_json(os.path.join(args.dataset_dir, scene_id, "scene_gt_info.json"))
    ...
    for im_id in im_ids_selected:
        ...
        selected_inst_ids = [inst_id for inst_id in range(len(im_gt_info)) if im_gt_info[inst_id]['visib_fract'] > 0.0]
        ...
        for obj_id in inst_per_obj_ids:
            test_list.append({"im_id": im_id, "inst_count": inst_per_obj_ids[obj_id], "obj_id": obj_id, "scene_id": int(scene_id)})

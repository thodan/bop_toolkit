...
def load_bop_objs(bop_dataset_path: str, model_type: str = "", obj_ids: list = [], sample_objects: bool = False, num_of_objs_to_sample: int = None, 
                  obj_instances_limit: int = -1, mm2m: bool = False, move_origin_to_x_y_plane: bool = False, temp_dir: str = None, config_file=None) -> List[MeshObject]:
    ...
    model_p = dataset_params.get_model_params(bop_path, bop_dataset_name, model_type=model_type if model_type else None, config_file=config_file)
...
def load_bop_intrinsics(bop_dataset_path: str, split: str = "test", cam_type: str = "", config_file=None) -> tuple[np.ndarray, int, int]:
    """
    ...
    from bop_toolkit_lib import dataset_params
    cam_p = dataset_params.get_camera_params(bop_path, bop_dataset_name, cam_type=cam_type if cam_type else None, config_file=config_file)
        
    try:
        split_p = dataset_params.get_split_params(bop_path, bop_dataset_name, split=split, split_type=cam_type if cam_type else None, config_file=config_file)
    except ValueError:
        raise Exception("Wrong path or {} split does not exist in {}.".format(split, bop_dataset_name))
